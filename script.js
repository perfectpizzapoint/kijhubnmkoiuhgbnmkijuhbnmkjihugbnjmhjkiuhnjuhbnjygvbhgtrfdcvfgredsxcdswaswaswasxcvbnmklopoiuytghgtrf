...
async function downloadSheetCache(silent = false) {
  ...
  try {
    ...
    // Store each response safely
    if (config) setCacheItem('getConfig', config);
    if (entries) setCacheItem('getAllEntries', entries);
    ...
    if (entries) ALL_ENTRIES_CACHE = entries;
...
function closeEntryForm() {
  ...
  localStorage.removeItem('ppp_checkout_table_id');
  resetPaymentMode();
  updateHomePendingItemsUI();
}

async function finalizeCheckoutTableCleanup() {
  const tableIdToClear = localStorage.getItem('ppp_checkout_table_id');
  if (tableIdToClear) {
...
function openSplitModal() {
  const amount = POS_STATE.currentBillTotal || 0;
...
function validateSplitTotal() {
  const total = POS_STATE.currentBillTotal || 0;
...
async function handleSaveEntry() {
  ...
  if (!mobile) {
    ...
    try {
      ...
      // Clear persistent entry data & finalize table cleanup only on success
      localStorage.removeItem('ppp_loyalty_form_open');
      localStorage.removeItem('ppp_loyalty_mobile');
      localStorage.removeItem('ppp_loyalty_amount');
      localStorage.removeItem('ppp_loyalty_message');
      localStorage.removeItem('ppp_pendingOrderItems');
      updateHomePendingItemsUI();
      await finalizeCheckoutTableCleanup();
...
function buildWhatsAppLink(result, mobile, amount, message, isLoyaltyEntry = true) {
  ...
  const el = document.getElementById('linkWhatsapp') || document.getElementById('posLinkWhatsapp');
  if(el) el.href = link;
}
...
function calculateFinalAmount() {
  ...
  let finalAmt = subtotal + taxAmt - discountAmt;
...
async function finalizePosOrder() {
  const mobile = document.getElementById('posMobile').value.trim();
  const amount = window.POS_FINAL_AMOUNT || 0;
  ...
  try {
    ...
    // Optimistically add new entry to ALL_ENTRIES_CACHE for instant update on Entries page
    const newEntryObj = {
      mobile: mobile || '',
      amount: amount,
      date: date,
      ...
      card: payAmts.cardAmt,
      orderItems: orderItemsStr,
      numEntries: (result && result.totalEntries !== undefined) ? result.totalEntries : '-',
    ...
    if (document.getElementById('step4Amount')) {
      document.getElementById('step4Amount').textContent = '₹' + Math.round(amount);
    }
    ...
    if (mobile && mobile.length === 10) {
      if (window.POS_REWARD_APPLIED) {
        try {
          await api({ action: 'claimReward', mobile: mobile });
          window.POS_REWARD_APPLIED = false;
        } catch(e) { console.error('Auto claim failed', e); }
      }
      
      const isLoyaltyEntry = typeof result.totalEntries !== 'undefined';
      buildWhatsAppLink(result, mobile, amount, message, isLoyaltyEntry);
...
async function handlePosReceiptOnly() {
  document.getElementById('posMobile').value = '';
  document.getElementById('posLoyaltyBadge').textContent = '';
  await finalizePosOrder();
}

async function handlePosClaimReward() {
  const mobile = document.getElementById('posMobile').value.trim();
  if(!mobile) return toast('Mobile required to claim reward', 'error');
  try {
    const res = await api({ action: 'claimReward', mobile: mobile });
    if (res.error) return toast(res.error, 'error');
    toast('🎁 Reward successfully claimed!', 'success');
    hide('btnPosClaimForce');
    if (typeof downloadSheetCache === 'function') downloadSheetCache(true);
  } catch (err) {
...
async function openStep4CustomerDetails() {
  ...
  try {
    const cust = await apiDirect({ action: 'getCustomer', mobile: mobile });
    if (cust && cust.found) {
      window.CURRENT_CUSTOMER = cust;
      openDetailsModal();
    } else {
      toast('Customer profile not found.', 'error');
    }
  } catch (err) {
...
function initiateEditEntry(index) {
  const entry = ALL_ENTRIES_CACHE[index];
  window.ADMIN_CONFIRM_ACTION = { type: 'edit', index: index, entry: entry };
...
async function submitAdminPasswordConfirm() {
  ...
  try {
    ...
    if (action.type === 'delete') {
      deleteEntryAction(action.entry || action.index);
...
async function deleteEntryAction(targetEntryOrIndex) {
  let entry = null;
  let indexInCache = -1;

  if (typeof targetEntryOrIndex === 'object' && targetEntryOrIndex !== null) {
    entry = targetEntryOrIndex;
    indexInCache = ALL_ENTRIES_CACHE.indexOf(entry);
  } else {
    indexInCache = Number(targetEntryOrIndex);
    entry = ALL_ENTRIES_CACHE[indexInCache];
  }
  ...
  try {
    ...
    // Success: remove from local cache using exact reference match
    const removeIdx = ALL_ENTRIES_CACHE.indexOf(entry);
    if (removeIdx !== -1) {
      ALL_ENTRIES_CACHE.splice(removeIdx, 1);
    }
    setCacheItem('getAllEntries', ALL_ENTRIES_CACHE);
    filterEntries();
    toast('Entry deleted successfully!', 'success');
    
    // Invalidate dashboard/re-sync in background
    if (typeof downloadSheetCache === 'function') {
      downloadSheetCache(true);
    }
  } catch (e) {
